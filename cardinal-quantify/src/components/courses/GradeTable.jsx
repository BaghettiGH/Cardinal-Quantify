...
function GradeTable({ setFinalGrade, mode }) {
    ...
    const estimateGrade = () => {
        const totalWeight = assignments.reduce((acc, assignment) => acc + (parseFloat(assignment.weight) || 0), 0);
        const totalGrade = assignments.reduce((acc, assignment) => acc + (parseFloat(assignment.grade) || 0), 0);
        const estimatedGrade = (totalGrade / totalWeight) * 100;

        if (estimatedGrade >= 70) {
            setFinalGrade(estimatedGrade);
        } else {
            setFinalGrade(70); // Set to 70 if below
        }
    };
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>
                            {mode === 'estimate' ? (
                                <button onClick={estimateGrade}>Estimate Grade</button>
                            ) : (
                                <CalculateButton assignments={assignments} setFinalGrade={setFinalGrade} />
                            )}
                        </th>
                    </tr>
                </thead>
